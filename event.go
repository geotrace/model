...
//
// Каждое событие получает свой уникальный идентификатор, назначаемый непосредственно системой.
// Кроме того, событие привязано к конкретному идентификатору устройства и группе пользователей.
// Группа пользователей здесь представлена отдельным свойством, не смотря на то, что ее можно
// достаточно легко получить и из связи с устройством. Это сделано намеренно, чтобы в тех случаях,
// когда устройство меняет владельца (группу), старые данные о событиях не становились
// автоматически доступны новым пользователям.
//
// Каждое событие в обязательном порядке характеризуется временем, когда оно произошло. Если при
// создании описания события время было опущено, то будет автоматически добавлено текущее время
// сервера.
//
// Тип события задает один из предопределенных типов события. Если не указано, то считается, что
// тип события не определен.
//
// Каждое событие обычно характеризуется координатами географической точки, в которой оно случилось
// и дополнительным параметром, указывающим возможный радиус погрешности вычисления данной точки.
//
// Дополнительно, каждое событие может иметь свое описание в текстовом виде и иконку,
// характеризующую его в некотором визуальном виде. Но с последним обычно тяжело: кто и сколько
// таких иконок нарисует? Поэтому было принято решения вместо иконки использовать пиктограмму из
// стандартного набора эмодзи.
//
// И, наконец, последний элемент: именованные поля с произвольным содержимым, позволяющим описать
// любую дополнительную информацию. В частности, думаю, значения датчиков и сенсоров хорошо и удобно
// сохранять именно в таком виде. Плюс, всегда можно добавить что-то дополнительно практически
// в любом удобном формате. Главное, чтобы приложение знало, что потом с этим делать.
type Event struct {
